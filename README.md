...
# 📈 個股查詢分頁（第二分頁）：K 線圖 + 法人買賣超表格

在頁面上方的 Tabs 中選擇 **「📈 個股查詢」** 即可切換至個股模式。  
此分頁提供完整的個股查詢功能，包含：
...
### 🕯️ 2. K 線圖（Lightweight Charts）
- K 線（OHLC）  
- 5MA / 10MA / 20MA / 60MA  
- 量能 Volume  
- 使用 `.no-scale-chart`，避免與桌機 `scale-150` 衝突  
- 資料來源：`Kline.json`（每日補齊最新資料）
...
### 🧩 4. AI 供應鏈分類（industry_tags.json）
若該股票位於最新 AI 伺服器供應鏈分類（如 ABF、PCB、CPO、散熱、伺服器 ODM…）  
會在個股頁面顯示對應標籤。
